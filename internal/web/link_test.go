...
// TestLinkPrefersTelemtsSelfReportedLink covers FIX (a): once the poller has
// a snapshot with a non-empty telemt link, both the list row and the detail
// page (and therefore the QR, which is derived from the same link) must show
// that link instead of the one the panel computed locally.
func TestLinkPrefersTelemtsSelfReportedLink(t *testing.T) {
	...
	h, auth, svc, pl := newLinkTestServer(t, "1.2.3.4", linksClient{links: []string{telemtLink}})
